...
port = serial.Serial('/dev/ttyS0', baudrate=9600, timeout=5) ## Enter serial port here
...
apnset = 'AT+SAPBR=3,1,"APN","mcinet"\r\n' ## Set your APN
...
def handleGPSmsg(GGAmsg, RMCmsg):
	...
	lat_dir=str(msg.lat_dir)
	lon_dir=str(msg.lon_dir)

	gps_qual=str(msg.gps_qual)
	gps_num_sats=str(msg.num_sats)

	horizontal_dil=str(msg.horizontal_dil)
	...
	port.write(('AT+HTTPPARA=\"URL\",\"https://api.thingspeak.com/update?api_key=VSKIFNK3X986380X&field1=' +str(timestmp)+ '&field2=' +str(latitude) + '&field3=' +str(lat_dir)+ '&field4=' +str(longitude) +'&field5=' +str(lon_dir)+'&field6='+str(altitude)+'&field7=' +str(gps_num_sats)+'&field8=' +str(speed)+'\"\r\n').encode())
	print("data send")
